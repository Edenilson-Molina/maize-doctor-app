  ...
  await database.write(async () => {
    const batch = SEED_SCANS.map((seed) =>
      scansCollection.prepareCreate((scan) => {
        ...
        scan.label = seed.label;
        scan.confidence = randomConfidence();
        scan.distribution = randomDistribution(seed.label);
        scan.lat = seed.lat;
        scan.lon = seed.lon;
        scan.temperature = seed.temperature;
        scan.humidity = seed.humidity;
        scan.synced = false;
