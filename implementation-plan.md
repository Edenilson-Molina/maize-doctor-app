...
### Fase 5 — Retroalimentación Colaborativa + Dataset Nacional ✅ COMPLETADA
- [x] `ScanDetail.tsx` (`detalle_e_inteligencia_colaborativa_v2`): formulario de corrección escribe en `corrections` (`createCorrection`, ya existía desde Fase 2), timeline de estados derivado de `scan.createdAt` + `correction.createdAt` + `correction.status` (no se guarda un log aparte). Se oculta el formulario una vez existe una corrección para el escaneo (sin cola de reportes múltiples en esta fase).
- [x] `Contribute.tsx` (`contribuci_n_al_dataset_nacional`): escribe en `dataset_contributions` (`createDatasetContribution`, nuevo) con copia local de imagen vía `savePhotoFile` (ahora parametrizado con subdirectorio/prefijo). El `<select>` de "Etiqueta de Diagnóstico" del mockup tenía opciones inventadas (p. ej. "Roya Polvosa"); se reemplazó por las 9 clases reales de `DiagnosisClass` usando un nuevo componente `ChipPicker` (reutilizado también en `ScanDetail`), siguiendo el patrón visual de chips que ya existía en `HistoryScreen`.
- [x] Navegación: se agregaron stacks anidados `HomeStack` (`HomeMain` → `Contribute`) y `HistoryStack` (`HistoryList` → `ScanDetail`), mismo patrón que `ScanStack` (Fase 4). Se conectaron los dos botones que hasta ahora eran callejones sin salida: el banner "¡Sé parte de la Ciencia!" de Home y los cards del Historial.
- [x] **Adaptaciones vs. el mockup** (decididas para no inventar datos ni funcionalidad fuera de alcance): sin campo condicional "Especifique el tipo" ("otro") en el picker de corrección — `Correction.observedLabel` está tipado estrictamente como `DiagnosisClass` y el textarea "Reportar Falso Positivo" ya cubre el texto libre. La tarjeta "Impacto Colectivo" muestra el conteo real de contribuciones de este dispositivo (`getContributionCount`), no el "5,000 agricultores" ficticio del mockup.
- [x] **Prueba de salida:** `ScanDetail.test.tsx` (4 tests: diagnóstico + N/D, envío de retroalimentación, formulario oculto con badge "Pendiente", timeline "Revisado" cuando `status: 'reviewed'`), `Contribute.test.tsx` (3 tests: botón deshabilitado sin foto/etiqueta, conteo real mostrado, flujo completo de captura→selección→contribución), `datasetContributionQueries.test.ts` (3 tests), `getScanById` agregado a `scanQueries.test.ts`. 75 tests totales en verde. Manual en dispositivo: tocar banner de Home → `Contribute` → elegir etiqueta → volver; tocar card de Historial → `ScanDetail` → enviar retroalimentación → ver badge "Pendiente" y timeline de 3 eventos — verificado en emulador Android.

**Notas técnicas resueltas durante Fase 5:**
- **`fireEvent.press`/`fireEvent.changeText` de RNTL v14 también son `async`** (mismo patrón que `render()`, Fase 4) — no esperarlos no lanza error, pero produce llamadas `act()` superpuestas que corrompen el estado de React entre interacciones y hasta entre tests subsiguientes del mismo archivo. Toda interacción simulada en los tests nuevos usa `await fireEvent....(...)`.
